...
from helper import open_, getIndex, load_dawg
import honeyvault_config as hny_config
from honeyvault_config import NONTERMINAL, TERMINAL, MIN_COUNT
from .lexer_helper import Tweaker, GrammarStructure
...
""", re.MULTILINE)
    ...
    def __init__(self, dawg=None, tweaker=None):
        ...
        if not self.dawg and \
            ...
            self.dawg = load_dawg(hny_config.DICTIONARY_DAWG)
        if not self.dawg:
            print("Oh My God!! You dont have the dictionary ({0})."
                  "I can't take it anymore. Exiting!"
                  .format(hny_config.DICTIONARY_DAWG))
            exit(0)
        self.infix = '@._*'
    ...
    def score(self, orig, wlist):
        ...
        dic_words = filter(lambda x: x in self.standard_english, wlist)
